-- 시설 업무일지: "미완료 항목 자동 이월" 기능을 위한 DB 변경 스크립트
-- 이미 운영 중인(D1) 데이터베이스에 새 컬럼 하나(carried_from)를 추가합니다.
-- 기존 데이터는 전혀 지워지지 않고, 새 컬럼은 모든 기존 행에서 빈 값(NULL)으로 시작합니다.
-- (경리 업무일지에 적용했던 migration_add_carried_from.sql 과 완전히 같은 방식이며,
--  이번에는 journal_entries 표에 적용합니다.)
--
...
--   npx wrangler d1 execute bms-money-db --remote --file=./migration_journal_carried_from.sql
--   (DB 이름이 다르면 실제 이름으로 바꿔주세요. --remote 를 반드시 붙여야 실제 운영 DB에 적용됩니다.)

ALTER TABLE journal_entries ADD COLUMN carried_from TEXT;
